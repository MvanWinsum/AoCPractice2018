...
def draw_straight_line(grid, line):
...
def draw_diagonal_line(grid, line):
    x_line = [int(line[0][0]), int(line[1][0])]
    y_line = [int(line[0][1]), int(line[1][1])]
    if x_line[0] > x_line[1]:
        x_step = -1
    ...
    x_iterator = x_line[0]
    y_iterator = y_line[0]
    while x_iterator != x_line[1] and y_iterator != y_line[1]:
        grid[y_iterator][x_iterator] += 1
        x_iterator += x_step
        y_iterator += y_step
    return grid
...
for line in instruction_list:
    if line[0][0] == line[1][0] or line[0][1] == line[1][1]:
        grid = draw_straight_line(grid, line)
    else:
        grid = draw_diagonal_line(grid, line)
